process generate_sample_report{
    ...
    input:
    tuple val(sample), path(kraken), path(fastp), path(coverage), path(mlst), path(amrfinder), path(bestRef), path(snps_23S), path(snps_16S), path(quin_res_vcf)
    ...
    echo "Kraken Classfication" >> ${sample}_report.out
    ...
    echo "\n---------------------------------------------------------------------------------------------------------" >> ${sample}_report.out
    echo "Sequence typing using MLST" >> ${sample}_report.out
    cat ${mlst} >> ${sample}_report.out
    echo "\n---------------------------------------------------------------------------------------------------------" >> ${sample}_report.out
    echo "FASTani to select the best reference" >> ${sample}_report.out
    cat ${amrfinder} >> ${sample}_report.out
    echo "\n---------------------------------------------------------------------------------------------------------" >> ${sample}_report.out
    echo "AMRFinder results" >> ${sample}_report.out
